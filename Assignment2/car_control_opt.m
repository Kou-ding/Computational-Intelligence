...
vertical_walls = [
    ...
    15, 0, 7;   % Wall at x=15 from y=0 to y=8
];
...
dv_range = [0 20];
dv_small = [0 0 0.3];      % Triangular MF for Small (S)
dv_medium = [0 0.5 1];     % Triangular MF for Medium (M) 
dv_large = [0.5 1 20 20];   % Trapezoidal MF for Large (L)

% Input 2: Horizontal distance (dh) - triangular MFs from 0 to 1  
dh_range = [0 20];
dh_small = [0 0 0.3];      % Triangular MF for Small (S)
dh_medium = [0 0.5 1];     % Triangular MF for Medium (M)
dh_large = [0.5 1 20 20];      % Triangular MF for Large (L)
...
for i = 1:length(theta_init)
    ...
    for j = 1:max_iterations
        ...
        delta_theta = evalfis(fis, [dv, dh, theta_error]);
        ...
        if norm(y - yd) < 0.1 && norm(x - xd) < 0.5 % use absolute distances
            break; % Stop if close enough to the goal
        end
    end
    
    % Plot the trajectory for the current initial angle
    % after optimizing the membership funciton parameters
    ...
    for w = 1:size(vertical_walls, 1)
        ...
        plot([x_wall, x_wall], [y_start, y_end], 'k-', 'LineWidth', 2);
    end
    ...
    for w = 1:size(horizontal_walls, 1)
        ...
        plot([x_start, x_end], [y_wall, y_wall], 'k-', 'LineWidth', 2);
    end
